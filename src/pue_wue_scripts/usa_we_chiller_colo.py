...
from simulation_functions.simulation_funs_dc import PUE_WUE_WE_Chiller_Colo
...
# IMPORT SAMPLE FILES
# ------------------------------------------------------------------
we_chiller_samples = pd.read_csv(os.path.join(SAMPLES_FOLDER, "usa_we_chiller_colo_samples.csv"))
...
for epw_file in os.listdir(EPW_FOLDER):
    ...
    # ------------------------------------------------------------------
    # RUN FOR EACH WE CHILLER SAMPLE
    # ------------------------------------------------------------------
    for idx, sample in we_chiller_samples.iterrows():
        results = []
        for row in tqdm(
            weather_df.itertuples(index=True),
            total=len(weather_df),
            desc=f"WE_Chiller_Colo Sample {idx + 1}",
            unit="hour",
        ):
            hour = row.Index
            T_oa = row.T_oa
        ...
        df_out.to_excel(writer, sheet_name=f"WE_Chiller_{idx + 1}", index=False)
