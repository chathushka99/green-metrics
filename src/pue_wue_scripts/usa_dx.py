...
from simulation_functions.simulation_funs_dc import PUE_WUE_DX
...
# IMPORT SAMPLE FILES
# ------------------------------------------------------------------
dx_samples = pd.read_csv(os.path.join(SAMPLES_FOLDER, "usa_dx_samples.csv"))
...
for epw_file in os.listdir(EPW_FOLDER):
    ...
    # ------------------------------------------------------------------
    # RUN FOR EACH DX SAMPLE
    # ------------------------------------------------------------------
    for idx, sample in dx_samples.iterrows():
        results = []
        for row in tqdm(weather_df.itertuples(index=True),
                        total=len(weather_df),
                        desc=f"DX_Sample {idx + 1}",
                        unit="hour"):
            hour = row.Index
            T_oa = row.T_oa
            RH_oa = row.RH_oa
        ...
        df_out.to_excel(writer, sheet_name=f"DX_{idx + 1}", index=False)
