...
from simulation_functions.simulation_funs_dc import PUE_WUE_Chiller
...
# IMPORT SAMPLE FILES
# ------------------------------------------------------------------
chiller_samples = pd.read_csv(os.path.join(SAMPLES_FOLDER, "usa_chiller_samples.csv"))
...
for epw_file in os.listdir(EPW_FOLDER):
    ...
    # ------------------------------------------------------------------
    # RUN FOR EACH CHILLER SAMPLE
    # ------------------------------------------------------------------
    for idx, sample in chiller_samples.iterrows():
        results = []
        for row in tqdm(
            weather_df.itertuples(index=True),
            total=len(weather_df),
            desc=f"Chiller_Sample {idx + 1}",
            unit="hour",
        ):
            hour = row.Index
            T_oa = row.T_oa
        ...
        df_out.to_excel(writer, sheet_name=f"Chiller_{idx + 1}", index=False)
